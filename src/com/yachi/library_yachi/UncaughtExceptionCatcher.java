...
public class UncaughtExceptionCatcher implements UncaughtExceptionHandler {
		...
		if (!handleException(ex) && mDefaultHandler != null) {
			...
			new Thread() {
				...
				public void run() {
					...
					Toast.makeText(iContext, "程序出错，错误报告已经发送!",
							Toast.LENGTH_LONG).show();
					// }
					Looper.loop();
	...
	private String saveCrashInfoToFile(Throwable ex) {
		...
		try {
			...
			VLog.e("an error occured while writing report file..." + e);
		}
		return null;
	}
	...
	private void collectCrashDeviceInfo(Context ctx) {
		try {
			...
			VLog.e("Error while collect package info" + e);
		}
		...
		for (Field field : fields) {
			try {
				...
				VLog.e("Error while collect crash info" + e);
			}
		}
	}
}
